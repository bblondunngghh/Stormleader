// Run 76 s1 — shared request helper (copy of r73 lib; login is RATE LIMITED, mint once per process).
import fs from 'fs';
...
export async function mint() {
  mintCount += 1;
  if (mintCount > 4) throw new Error('REFUSING to mint a 5th time — login is rate limited (~10 tries -> 15 min lockout)');
...
export function summarize(body) {
  if (body && typeof body === 'object') {
    return Array.isArray(body) ? `array[${body.length}]` : Object.keys(body).slice(0, 8).join(',');
  }
