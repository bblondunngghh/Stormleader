...
### Test Results
- Pages tested: **25** routes (19 authenticated + **6 public, rendered for the first time in the pipeline's history**) + 15 Settings tabs + 4 modal interiors + 4 inline builders
- API endpoints tested: **18 distinct endpoints / 26 probes live**; **109** routes with path params statically set-differenced; inventory **280** route patterns across 37 files (135 GET, 90 POST, 27 PATCH, 20 DELETE, 8 PUT)
- Bugs found: **4** (1 backend, 3 UI)
- Bugs fixed: **4** (5 fix commits — the heading defect needed two)
- UI inconsistencies found: **3**
- UI inconsistencies fixed: **3**
- Net DB rows written: **0** (verified: 0 rows in 2h, milestone row restored, 0 milestones with `completed IS NULL`)
- Build: **PASS** (vite, 7.92s)
- Page errors: **0** on 19/19 authenticated routes and both `/status/:token` branches

### Fixes Made
- `5a3978e` — **`PATCH` milestone: a photo-only update wiped the completed flag.** `updateMilestone` bound `req.body.completed` straight into `SET completed = $3`; on a partial PATCH the field is `undefined`, node-postgres sends **NULL**, so the flag was overwritten and `completed_at` dropped by the CASE. **Live user path**: `WorkOrdersView.jsx:97` uploads a milestone photo with `{ photo_url }` alone, so **attaching a photo to a finished milestone marked it incomplete**. Fixed with `COALESCE($3::boolean, completed)`. ⚠️ **The end-to-end repro passed 4/4 on first run and that was MISLEADING** — s1 had already restarted the server (file mtime 05:10:12 vs server start 05:10:32), so the "before" was the post-fix state. **Always compare file mtime to server process start time before trusting a before/after.** Proven instead by a SQL differential on one real row: OLD wipes, NEW preserves. Found by s1 (capped), recovered and committed by s2.
- `5b73c95` — **the 6th public page still had no heading.** `c56f3d4` covered 5 of 6; `/status/:token` is reachable only with a real share token, so it could not be rendered until one was resolved from the DB. s4 wrote the lookup and the fix, then capped before verifying or committing. Recovered and verified in s5 across a stash/restore of the exact diff: **before 0 headings, DIV 462x33 @ top 73, innerText 246 → after 1 h1, H1 462x33 @ top 73, innerText 246.** Byte-identical text, pixel-identical box.

### UI Consistency Fixes
- **The 7 prescribed audits are converged for the 11th consecutive run — and were NOT the source of any finding.** Cheap regression only (source greps, definitive; plus one computed-style sweep): 0 foreign icon libs, 0 non-outline Heroicon imports, 0 native `<select>`, 0 date/time inputs, topbar 56px + `topbar glass` and 1 `<h1>` on 19/19, 18 nav links / 18 icons on 19/19, 0 overflow, 0 modals open at rest, 1,033 buttons consistent. **All three defects came from dimensions no prior run had measured.**
- `682e4cc` — ⚠️ **THE HEADLINE: INTERACTION STATES WERE NEVER MEASURED — only the default state.** One `:focus-visible` check found the largest-blast-radius UI bug in many runs. `index.css:4384`'s bare `:focus-visible` set `border-radius: var(--radius-sm)`, and **a bare pseudo-class has class-level specificity (0,1,0)** declared after every component rule, so it won the cascade the moment a control took keyboard focus. **204 elements in 10 classes changed shape on focus**: `.nav-link` 12→8px (**x90, every sidebar item on every page**), `.quick-action-btn` (x55), `.topbar__btn` (x10), `.auth-btn`, `.form-input`, and square controls like `.nav-group__header` rounding 0→8px. The declaration was also **unnecessary** — browsers already derive the outline radius from the element's own. **Verified 10 distinct changes → 0 with the ring still on 270/273 — ALWAYS assert the ring survives**, deleting the a11y indicator would be the worse bug.
- `b8b462c` — **same bug family: `[class*="dropdown"]` is a SUBSTRING match**, so it animated every BEM child. Notifications ran **4 simultaneous scale-ins** (panel + `__header` + `__list` + `__empty`), children scaling inside the scaling parent, each with a different resolved `transform-origin` (180/189/190px — `top center` resolves per element width). It also matched `.map-controls__dropdown`, an **always-visible wrapper**, and because `animation-fill-mode: both` holds `scale(1)` it kept a **permanent transform** — permanently creating a stacking context around a `.glass` child with `backdrop-filter`, **exactly the documented `feedback_no_animation_on_glass_parents` breakage.** It also silently overrode 3 components' own animations (`fadeIn` x2, `dropdown-fade-in`); unifying is the rule's intent so that was kept, now applied once per container. Replaced with an explicit container list.
- `c56f3d4` — **the 6 PUBLIC routes had never been rendered by any audit** (the documented "19 routes" sweep is authenticated-only). All 6 render clean, but had **zero `h1`/`h2`/`h3`/`h4` between them** while all 19 app routes carry an `<h1>`. "Sign in", "Create your account", "ESTIMATE EST-090", "CONTRACT" were plain `<div>`s — the pages homeowners actually see. Promoted the existing title element on each; no new markup, no new CSS. **Visually neutral by construction** (universal reset at `index.css:13` zeroes UA heading margin; the classes set size and weight explicitly) and **proven by byte-identical `document.body.innerText.length` on all five pages (113/156/189/234/1089)**.
- `a97fe00` — new **self-tested** harness `server/.qa-r92-broadselector.mjs`: finds selectors that are **broad but carry class-level specificity** (bare pseudo-class, `[attr*=]`) while setting component-owned properties. **`--selftest` runs it against `682e4cc~1` and requires it to rediscover BOTH of tonight's proven defects — it catches both out of 636 rules.** 6 findings remain on the fixed tree, all benign: bare **element** selectors (0,0,1 — always lose to a class) and scrollbar pseudo-elements. **The distinction that matters is element-vs-pseudo-class breadth, not "broad" alone.**
- `ce8bf6b` — `.qa-r91-validategap.mjs` set difference (each route's real path params vs what its `validateId()` guards): **109 routes → 0 category-A, 0 category-B. The class does not exist here.** The zero is credible **only because it was self-tested first** against two planted defects, which it caught. Plus `.qa-r91-badid.mjs` (the 13 routes with no `validateId` answer 400/404 on non-UUID/numeric/injection ids, no 500s; 8 of 13 take `:token` and are correct by design), `.qa-r91-dbcheck.mjs`, `.qa-r91-mint.mjs`.

### Known Issues Remaining
- **THE FULL API SWEEP DID NOT RUN THIS CYCLE.** s1-api-test capped at 51/50 before executing anything. Last full 280-pattern sweep: **Run 82, 2026-08-19**. Tonight's backend coverage is 18 endpoints live plus static analysis — **a gap, not a pass.**
- **Three authored API harnesses are STILL UNRUN, no output files exist.** `.qa-r91-realidwrite.mjs` is **the highest-value remaining item in the whole pipeline** — 34 PATCH/PUT handlers have never executed against a real row, the documented blind spot behind two of the last five real bugs. **It performs writes: read it before running, never execute blind at the end of a stage.** Also `.qa-r91-neverrun.mjs` and `.qa-r91-validateid.mjs` (superseded by validategap, which ran clean).
- **`:hover` and `:active` are still unmeasured** — the same shape of gap that `:focus-visible` was for 11 runs before it paid out immediately.
- A dead-UUID sweep 404s **inside validation, before the handler body** — it proves validation, not handlers. Run 87 got a real id into only **29 of 119** param GET routes.
- 4 GET handlers have never executed their bodies (`drip_sequences`, `prospect_lists`, `financing_applications` hold zero rows).
- Modal interiors only partly covered — 4 of 22 backdrops opened.
- `/alerts` = 0 `.is-active` sidebar entries (orphan route); `.glass` padding drift (Tailwind rem vs 14px root); modal title heading drift (`/materials` h3/16px vs `/work-orders` h2/18px, `/expenses` h3) — all cosmetic, deferred.
- The 2 duplicate CSS blocks — **verified, not assumed**: all custom props defined, both resolve to the intended value. Dead declarations only.
- `ExpensesView.jsx:56` dead `searching` state — real but cosmetic.
- Dead files on disk: `client/src/components/Icons.jsx.backup` and `client/src/assets/icons-backup/` (17 duplicated SVGs). Not imported, no runtime effect — noted so a future icon audit does not re-flag them.
- No delete anywhere in the app asks for confirmation (`confirm(` appears **zero** times) — app-wide and apparently deliberate; a design decision.
- Pre-existing QA rows now user-visible (`qa_options_probe` custom field, lead `Qa20260730c`) — not from this run, irreversible to delete, **left for the user's call**. Two orphaned QA upload files on disk (23 bytes each); DB rows already removed.
- Skip-trace 503 without `TRACERFY_API_KEY` (intentional). County import `ENOTFOUND feature.tnris.org` (no network access to that host here). This repo has **no ESLint at all**, which is why the committed `.qa-*.mjs` set differences exist.
- **RESOLVED this run — do not re-file: `/admin`'s 403 is correct authorization.** It sat on the "documented-intentional" list for five runs on an *unproven* hypothesis. s2 ran the probe s4 wrote and never got to: `super_admin` → **200** on all four admin endpoints, `admin` (what the browser actually sends) → **403**. The page renders only because `VITE_DEV_BYPASS_AUTH=true` shows a `super_admin` `DEV_USER` in the SPA. **Moved to verified non-bug.**

### Notes for the next run
- **`git status` FIRST — it paid out TWICE tonight**, and on 6 of the last 9 nights. s1 capped at 51/50 holding a complete uncommitted fix (recovered by s2 as `5a3978e`); s4 capped at 41/40 holding another (recovered by s5 as `5b73c95`).
- **THE STATIC 7 UI AUDITS ARE CONVERGED (11 RUNS) AND PRODUCED NOTHING AGAIN. Do not spend a run on them.** The cheap regression is source greps (definitive — a browser sweep cannot see inside an unopened modal) plus one computed-style sweep; that is 3 turns. **Spend the run on an UNMEASURED DIMENSION — that is where all three of tonight's defects came from, after 10 straight "converged" runs.**
- **NEXT-BEST UI GAP: `:hover` and `:active`.** Check specifically for hover rules that change **geometry** — padding/border-width/height cause layout shift; transform does not.
- **THE BUG FAMILY TO KEEP HUNTING: broad selectors with class-level specificity.** Both CSS defects were this shape. `.qa-r92-broadselector.mjs` now detects it and is self-tested against both.
- ⚠️ **TRAP — AN AUTOFOCUSED INPUT LOOKS EXACTLY LIKE A ROGUE ONE-OFF STYLE.** `/register`'s COMPANY NAME `.form-input` measured 8px radius + lighter bg + blue border against four identical siblings at 12px, with no inline style in the source to explain it. It was `:focus` — the field is autofocused on mount. **Before filing ANY single-element style outlier: check `document.activeElement`, blur it, re-measure.** (Productive trap — it is what led to the real `:focus-visible` bug — but file nothing until you have blurred.)
- ⚠️ **TRAP — compare file mtime to server process start time before trusting any before/after API result.** Tonight's milestone repro passed 4/4 "before" the fix because the server had already been restarted with it.
- **A set difference reporting ZERO is worthless without a self-test.** Both of tonight's clean zeros (validategap: 109 routes → 0; broadselector on the fixed tree) are credible only because planted defects were caught first.
- Public routes are now covered and clean — re-check only if they change.
- **Four of five stages capped again — the sixth consecutive night.** Recovery works but is costing roughly a stage of throughput per night.
- Drift baseline for next run: the `docs: QA report 2026-08-24` commit (head after this entry).
...
**Baseline `80c54c6` (tag `pre-overnight-20260825`) → head `9339aa4`. Build PASS (vite 7.93s). Net DB writes 0.**
**Stages: s1 api-test (Run 95) CAPPED · s2 frontend-test (Run 95) CAPPED · s3 ui-audit (Run 94) COMPLETED · s4 verify (Run 96) CAPPED · s5 report.**

### Test Results
- Pages tested: **18 authenticated routes swept** (UI audit). **0 pages have a frontend-stage coverage artifact** — s2 capped without writing one.
- API endpoints tested: **258 of 272 route patterns executed live** (94.9%); **PATCH/PUT 34 of 34 against real rows — a pipeline first**; 14 excluded by design (bulk import, outbound email, paid geocoding, webhooks). 0 5xx anywhere.
- Bugs found: **6** (1 backend, 5 frontend/UI)
- Bugs fixed: **6**
- UI inconsistencies found: **1**
- UI inconsistencies fixed: **1**
- Also: **1 harness bug found & fixed** (jsonb-corrupting restore, which had silently corrupted one real row — repaired), and **1 change made then reverted** (`323bf72` → `db6c7d5`).

### Fixes Made
- `d49813c` — **`/api/leads?needs_followup=true` returned 400 for EVERY tenant on EVERY request.** `leadService.getLeads` compared the `lead_stage` enum column directly against `'closed_won'` and `'closed_lost'` — **neither label exists in the enum** (`new, contacted, appt_set, inspected, estimate_sent, sold, lost, negotiating, in_production, on_hold`), so Postgres raised `22P02` and the handler turned it into a hard 400. The filter could never return a row. Compared as text instead, matching the idiom already used by the stage filter a few lines above. **400 → 200, total=13**, verified against SQL ground truth. ⚠️ **An empty-query GET sweep is structurally incapable of finding this — the crash lived in a query param no sweep was passing.** This is the argument for parameterised sweeps.
- `a04aa8e` — **estimate-to-contract conversion dropped the lead's email and phone.** ContractBuilder's `fromEstimate` prefill fell back to `lead_name`/`lead_address` but not `lead_email`/`lead_phone`, though the detail query joins all four in for this purpose (`estimateService.js:164-165`). **Send Contract is gated on `customerEmail`** (`ContractsView.jsx:505`), so the flow's primary action was unusable without retyping data the app had already fetched. Proven with a response-merge intercept, zero DB writes.
- `716471a` — **work-order estimate picker read two keys the API never sends.** `EstimatePickerModal` rendered `est.title` and `est.contact_name`; the query aliases them `estimate_name` and `lead_name` (`estimateService.js:122`), so both were `undefined` and all 16 rows fell to their fallbacks — a user-named estimate showed as a generic `Estimate #EST-0xx` and the customer name never rendered. The correct idiom was one file away in the invoice picker (`InvoicesView.jsx:372-374`).
- `be61054` — **contract lead-search dropdown showed a bare dash for EVERY result.** Rendered `lead.owner_name || lead.first_name || '—'`; neither key is on the lead payload, so only the address underneath carried information. `BAISDON` → **BAISDON HANNAH**, `TYNES` → **TYNES RAYMOND L**. The same file already documented this exact trap at `:316-331` — the dropdown had been missed by that earlier fix.
- `f9b0c8f` — **picking a lead for a contract populated only the address.** `selectLead()` read `owner_name`/`first_name`/`email`/`phone`, all absent, so name/email/phone were set to `''` and **Send Contract stayed disabled**. ⚠️ **THIRD SITE IN THE SAME FILE with the same defect** — two were fixed earlier the same night. When a key-mismatch is found, sweep the whole file, not just the reported call site.
- `2a6972f` — **harness fix: node-postgres binds a JS ARRAY as a Postgres ARRAY LITERAL, not JSON.** `restoreRow` read `jsonb` back as a JS value and bound it straight, so restoring `line_items = []` wrote `'{}'` — a jsonb **OBJECT**. It **silently corrupted `work_orders` f8a24416 during the run** (caught by the restore verifier, repaired to `[]`; table back to array=10, 0 objects). The same latent defect sat in `.qa-r91-realidwrite.mjs` and only failed to fire because nothing had ever mutated. Both now serialize to text and cast `::jsonb`. Self-tested on the exact row that corrupted.
- `9d0ca27` — **closed the last 10 write handlers; PATCH/PUT is now 34/34.** Every prior sweep topped out at 24: 7 backing tables held no row for this tenant (so the request 404'd in the ownership lookup **before the handler body ran**) and 3 state transitions were held back. Each is now seeded with one tenant-owned row, PATCHed on a **real** field, asserted changed, then deleted — not exercised with an empty body.
- `40f04a1`, `9339aa4` — two self-tested QA harnesses (cross-entity key set difference; JS hover-handler symmetry).

### UI Consistency Fixes
- `25f5472` — **THE NIGHT'S UI DEFECT: `.stat-card`'s explicit `transform` opt-out was dead.** `.stat-card:hover` (`index.css:949`) declares `transform: none` **deliberately** — the card carries `backdrop-filter: blur(40px) saturate(1.5)` *directly*, so a transform makes it a stacking context, the same family as the 2026-03-29 dashboard glass-card regression. The late generic utility `.glass[class*="card"]:hover` (`:4339`) beat it **twice over — declared 3,390 lines later AND more specific (0,3,0 vs 0,2,0)**. Measured with a real mouse: `before hovered: matrix(1,0,0,1,0,-1)` → `after hovered: none`. Fixed with `:not(.stat-card)`; `.glass.report-card` on `/reports` still lifts and still gains its hover shadow.
- ⚠️ **THIRD CONSECUTIVE RUN WITH THE SAME DEFECT SHAPE: a broad selector declared LATE wins the cascade against a component's own decision.** (Run 92: bare `:focus-visible`, `[class*="dropdown"]`; Run 94: `.glass[class*="card"]`.) **Triage this shape FIRST.**
- ⚠️ **REUSABLE CHECK THAT FOUND IT: a property set to `none`/`0`/`initial` inside a `:hover`/`:focus`/`:active`/`:disabled` block is a DISTRESS SIGNAL.** Nobody writes `transform: none` in a hover rule to mean "do nothing" — it exists **only** to cancel something, so it is load-bearing and the cascade may be silently ignoring it. Cheap, general, and it paid out immediately.
- **The static 7 regressed CLEAN on 18 routes in ~3 turns — 12th consecutive converged run. Do not re-spend a run on them.** Topbar 56px + `.glass` 18/18; one `<h1>` 18/18; 18 nav links/icons 18/18; 0 native `<select>`; 0 date/time inputs (source *and* runtime); 0 overflow; 0 overlays open at rest; 575 buttons all mapping to documented per-context families; 0 foreign icon libraries.
- **Three dimensions now CLOSED (measured, clean):** hover geometry reflow **0 of 47** rules; sticky hover in JS **0 of 33** tags (**negative side verified, not assumed**); the broad `button:not(...):active{transform:scale(.97)}` is **SAFE** — **0 transform-positioned and 0 glass-parent buttons out of 575**.

### Known Issues Remaining
- **`qa_options_probe` custom field definition is still in the production DB** (tenant `waterloo`, created 2026-08-05, label "QA Options Probe") and is **user-visible in Settings → Custom Fields**. The r95 hygiene sweep matches `'QA-R9%'`, which this label does not match, so it escapes cleanup every night. **NEW EVIDENCE — it is safe to delete: 0 leads carry a value under that key and 0 leads have ANY `custom_fields` data at all.** Needs one `DELETE` plus a developer's go-ahead; the report stage does not write to the production DB.
- **s4's 7 verification harnesses are UNCOMMITTED** (`server/.qa-r96-s4-*.mjs`), left behind when the stage capped. ⚠️ **`.qa-r96-s4-payloadkeys.mjs` HAS A WRONG-PATH BUG** — it requests `/api/crm/estimates`, which **404s**; the router is mounted at **`/api/estimates`** (`routes/index.js:62`). That 404 produced **2 spurious assertion failures** that read exactly like real UI defects. **Fix the path before committing or trusting it.**
- **`/tmp/api-test-results.txt` WAS NOT UPDATED — it is still the Run 82 file dated 2026-08-19.** Commit `9d0ca27` claims three tester-error traps are "documented in /tmp/api-test-results.txt"; **that documentation was never written.** The traps survive only in the commit messages and in OVERNIGHT-REPORT.md. **s2 produced no results file at all.** Do not read that file as current.
- Dead code: `quickFilters`/`applyQuickFilter` (`LeadList.jsx:62`, `:346`) — defined, never rendered, never called. Dead CSS: `.stat-card:hover .stat-card__icon img` (`index.css:959`) — `.stat-card__icon` has 0 JSX consumers. Both developer calls.
- 14 of 272 routes remain deliberately unexecuted (bulk import, outbound email, paid geocoding, webhooks) — an intentional boundary, not a gap.
- Stray 0-byte `server/=` in the working tree (shell-redirect accident, pre-existing).

### Verified non-bugs — do not re-file
- **`/api/leads?stage=sold` → 0 rows is DATA DRIFT, not a regression.** SQL ground truth: this tenant is 9 `new`, 2 `contacted`, 1 `appt_set`, 1 `inspected` = 13, and **zero** in `sold`. The label is valid; no row carries it.
- **The 2 "assertion failures" from `.qa-r96-s4-payloadkeys.mjs` are tester error, not defects** — see the wrong-path bug above. Re-run against `/api/estimates`, every key is present and both fixes verify.
- `.glass[class*="card"]` matches **only** `stat-card glass` and `glass report-card` — no BEM children, no glass descendants. Latent per the Run 92 `[class*=]` rule, **not a live defect**.
- Four API tester-error traps: financing routes take **camelCase** (`isActive`/`isDefault`) while the rest of the API is snake_case (verified correct against the real caller `SettingsView.jsx:1936`); notifications' column is **`is_read`**, not `read`; `markRead` scopes by `req.user.id`, so seeding against an unordered `LIMIT 1` user yields a **CORRECT 404** that reads like a broken endpoint; `custom-fields` is backed by **`custom_field_definitions`**.
- `/admin`'s 2 console errors are the proven-intentional 403s.

### Notes for the next run
- **`git status` FIRST.** It paid out again: s4 capped holding 7 uncommitted harnesses, recovered and executed in s5. It has now paid out on 7 of the last 11 nights.
- **THREE OF FOUR STAGES CAPPED AGAIN — the seventh consecutive night.** Recovery works but is costing roughly a stage of throughput per night. s2 burned 81 turns and left no artifact.
- **NEXT-BEST UI GAP: MODAL INTERIORS**, plus `:disabled` / `:checked` / `[aria-expanded]`. Default, `:focus-visible` (R92) and `:hover`/`:active` (R94) are now **all measured** on the 18 top-level routes. A violation inside an unopened modal is invisible to every runtime sweep run so far. **Source greps beat browser sweeps for this — a browser cannot see inside an unopened modal.**
- **NEXT-BEST FRONTEND TARGET: sweep the remaining views for the key-mismatch class.** It produced **4 of tonight's 6 defects**, 3 of them in one file. `.qa-r95-loopkeys.mjs` (loop-variable resolution) and `.qa-r95-varkeys.mjs` (variable-name resolution) are both committed and self-tested — point them at the views s2 never reached.
- **`.qa-r91-realidwrite.mjs` FINALLY RAN after four blocked nights, and it was worth it** — it exposed the jsonb-corruption bug in its own restore path. **The write sweep is no longer the top unrun item.** `.qa-r91-neverrun.mjs` is still unrun.
- **Seed fixtures before re-verifying tonight's UI fixes.** No lead in this tenant has a `contact_email` and the sampled lead-linked estimate has null `lead_email`/`lead_phone`, so `f9b0c8f` and `a04aa8e` verify **structurally only** — the correct keys are present on the payload, but the fix cannot be demonstrated end-to-end. Likewise `needs_followup` returns 13/13 (all 13 leads pass the predicate), so **it cannot currently be proven to filter anything**.
...
**Branch** `feat/financing` · **Baseline** `cfa8d0c` · **Head** `95b9cab` · **Build** PASS (vite 7.87s, exit 0) · **Net DB writes** 0

### Stage completion
| Stage | Run | Terminal reason | Turns |
|---|---|---|---|
| s1 api-test | 100 | **`max_turns` (50)** | 51 |
| s2 frontend-test | 101 | `end_turn` | 81 |
| s3 ui-audit | 102 | `end_turn` | 73 |
| s4 verify | 103 | `end_turn` | 47 |

**Three of four test stages completed on task — the best stage-completion result in this pipeline's logged history**, and the first s4 ever to finish its edge-case pass. ⚠️ **s1's own resume entry claims `Stage capped? NO`, which is FALSE** — its stage JSON records `subtype: error_max_turns` at turn 51. It capped during wrap-up, after writing its artifact and resume entry but **before committing 12 `.qa-r100-*` harnesses**, which s4 swept up in `95b9cab`. Its findings are unaffected; its self-report is not reliable. **Read the stage JSON, not the stage's self-assessment.**

### Test Results
- Pages tested: **19 of 19** authenticated routes (+ 15 Settings tabs, 8 create flows)
- API endpoints tested: **254 of 272** route patterns (93.4%), **663 requests** — up from 133 on the prior run
- Bugs found: **3**
- Bugs fixed: **3** (100%)
...
- 5xx / crashes: **0** · `pageerror`: **0** · malformed-input fuzz: **17/17 → 4xx, 0 → 5xx**
- Commits: **6** — 3 fixes (`29c009a`, `add652c`, `50fde1b`), 3 harness commits (`f9f08b3`, `08d14b0`, `95b9cab`)

### Fixes Made
- `29c009a` — **`PATCH /api/crm/leads/:id` accepted a non-object `custom_fields` and PERMANENTLY corrupted the column.** `crmService.js:195` merges with jsonb `||`: `custom_fields = COALESCE(custom_fields,'{}') || $n::jsonb`. ⚠️ **Postgres does NOT raise on `object || scalar` — it silently returns an ARRAY**: `'{}'::jsonb || '"a-string"'::jsonb` => `[{}, "a-string"]`. The route validated `priority` and `stage` (`crm.js:183-190`) but **nothing validated `custom_fields`**, so a body-controlled string/number/boolean/array/null was accepted with **200**. **The damage is permanent AND escalating** — once the value is an array every later *legitimate* save appends instead of merging (`[{},"junk"] || '{"k":"v"}'` => `[{},"junk",{"k":"v"}]`), and `LeadDetail.jsx:1285` reads `lead.custom_fields?.[def.field_key]`, which is `undefined` on an array, so **every stored custom field silently vanishes from the UI**; `:1287` then spreads the array into an object, writing numeric keys. All five non-object types were accepted. Fixed with a 400 guard matching the existing priority/stage pattern. Verified post-restart (server start `05:07:20` > `crm.js` mtime `05:06:56`): **6/6 bad types now 400**, valid object still 200s, two successive merges compose correctly. The one production row corrupted while reproducing was repaired to `{}`; **0 of 13 leads now hold a non-object `custom_fields`**.
- `add652c` — **two disabled buttons rendered at FULL OPACITY and looked clickable.** The inline `opacity` covered only the *in-flight* reason for being disabled, not the validation reason. `SettingsView.jsx:1518` — `disabled={!stripe || processing}` vs `opacity: processing ? 0.6 : 1`; since `loadStripe(VITE_STRIPE_PUBLISHABLE_KEY || '')` never resolves when that env var is absent, "Add Payment Method" renders as a normal enabled blue button with an unchanged label and does nothing on click. `SettingsView.jsx:515` — same shape on the plan switcher. Proved by **DOM injection rather than by driving either write path** (zero DB writes): disabled `.auth-btn` with no inline opacity computes `0.5`; the same button with inline `opacity:1` computes `1` while `pointer-events` stays `none`. Fixed to match the idiom already correct at **26 other sites**.
- **s2 also CLOSED the gap Run 99 named and carried for three nights** — the inline-`style`-vs-`:hover` diff, app-wide: **3,353 elements swept, 0 fully-dead states.** The 27 sites that lose a hover *colour* all keep `transform: translateY(-1px)` (runtime-confirmed on the worst case: `none → matrix(1,0,0,1,0,-1) → none`). ⚠️ **The defect came from EXTENDING that check to `:disabled`, not from running it as written** — `button:disabled` is an *element* selector that never appears in a className diff, so it was invisible to every prior sweep. **The sweep now covers `:hover :focus :focus-visible :focus-within :active :disabled :checked` and is CLOSED.**

### UI Consistency Fixes
- `50fde1b` — **the `/storm-map` time-range dropdown had NO hover state at all.** `index.css:2772` `.map-controls__dropdown-trigger` held a **verbatim copy-paste of the `.glass` block** with `!important` added to two declarations — on an element that already carries `className="map-controls__dropdown-trigger glass"` (`MapControls.jsx:38`), so the copy was **pure redundancy**. But `!important` inverts the cascade: it beats higher specificity, later source order **and inline styles**. Those two lines silently killed **(a)** the rule's own component-specific background/border five lines below and **(b)** the ENTIRE `:hover` rule (`:2797`), both declarations. Unlike the 27 documented colour-locks, this hover rule has **no `transform` to fall back on** — zero feedback. Runtime proof with a real `page.mouse.move` (`hoverCount 11`, `matches(':hover') === true`): background and border **byte-identical at rest and hovered**. An injected probe confirmed the mechanism independently — even an *inline* hover colour computed back to `--glass-bg`. Design intent confirmed first: the sibling `.map-controls__dropdown-menu` (`:2811`) declares a border byte-identical to the trigger's suppressed one, and the hover pair is an obviously authored progression. **Fix is 4 deletions, 0 additions.** Box unchanged 95×38, no layout shift.
- ⚠️ **WHY SIX PRIOR SWEEPS MISSED IT:** Runs 92/94/98/101 all attacked this same "a declaration silently loses" family but **all keyed on specificity or source order**. `!important` beats both, so every one of them scored a `base{p:!important}` / `base:hover{p}` pair as *"the hover wins"* and passed it.
- **Audits 1–7 regression-clean on 18 routes in ONE browser call.** Icons: 0 solid heroicons, 0 foreign libraries, 0 native icon SVGs outside the 2 documented map files (source AND runtime, 18/18). Buttons: every radius maps to a documented family. Headers: `.topbar` 56px + `.glass` 18/18, exactly one `<h1>` 18/18 with correct text. Nav: 18 links / 18 icons 18/18, `.is-active` = 1 on 17/18 (`/alerts` is the documented orphan). Forms: **0 native `<select>`, 0 `input[type=date]`** source and runtime. Spacing: 0 horizontal overflow 18/18. Modals: 0 overlays open at rest 18/18. **CONVERGED — do not re-spend.**
- **Three never-run checks added, all self-tested with negative controls** (`08d14b0`): shorthand-after-longhand (5 hits, **0 in-charter defects**); **0 duplicate JSX attributes and 0 duplicate inline-style keys app-wide** (a duplicate `onClick` is a 100% dead handler no rendering check can see); and the mirror of Run 101 — **59,152 inline declarations probed across 19 routes for `!important` overrides, 0 app defects** (only a mapbox sub-pixel artifact and the deliberate recharts dark-mode legend override).
- ⚠️ **A NAMED GAP WAS RETIRED BY ONE `grep -c`, NOT BY A RUN.** Run 101 named "inline style vs `@media` / `[data-*]` / `[aria-*]`" as the top unrun check. `index.css` is the app's **only** stylesheet and has **exactly one `@media`** (mobile, paused) and **ZERO attribute selectors**. **Standing lesson recorded: measure a gap's surface area before budgeting a run for it.**

### Verification (s4, Run 103) — 3/3 PASS, 0 new defects
- `29c009a` **PASS** — 6/6 bad types 400; happy path proven with an **identity merge (`custom_fields: {}`)**, which is value-preserving, so 200 was demonstrated with **zero data change**. Regression harness committed: `server/.qa-r103-s4-verify1.mjs`.
- `add652c` **PASS** — caught the transient `{disabled:true, inline:'0.6'}` → `{disabled:false, inline:'1'}`. ⚠️ **Only by reading `el.style.opacity`, NOT the computed value**: `.auth-btn` has `transition: all .15s`, so a computed read as `stripe` resolves returns a mid-transition `0.6` on an already-ENABLED button and fakes an inconsistency.
- `50fde1b` **PASS** — verified in the **BUILT bundle**, not just dev. ⚠️ **`bordersMatch:false` right after clicking the trigger is an artifact** (the trigger is still hovered), not a regression.
- **Edge cases, all clean and complete for the first time:** nav 18/18 (each moves the URL, renders an `<h1>`, leaves exactly 1 `.is-active`), back/forward 4/4, `/leads` search → "No leads found", `/tasks` empty state renders, **3 create forms submitted fully empty fired ZERO write requests**, 375px `scrollWidth === clientWidth` on 8 routes. Console: **2 errors app-wide**, both the documented `/api/admin/overview` 403.

### Known Issues Remaining
- **`automationEngine.js:75-77` `lead_priority` enum bug — NOW 8 RUNS OLD, has a written repro, and keeps being skipped because NO STAGE IS EXPLICITLY ASSIGNED IT.** It is not frontend-reproducible (s2 confirmed the Tasks UI side is clean), so the frontend stages cannot take it, and the API stage has never been told to. **Highest-priority carried item — assign it to an api-test stage by name.**
- **`qa_options_probe` custom field is STILL live in the production DB** (tenant `waterloo`, created 2026-08-05, label "QA Options Probe"), user-visible in Settings → Custom Fields. The hygiene sweep matches `'QA-R9%'`, which this label does not match, so it escapes cleanup every night. Confirmed safe to delete (0 leads carry a value). **Needs one `DELETE` + developer go-ahead — not QA's call.**
- **Form label drift — DEFERRED, developer decision, 5th run.** Canonical `.form-group label` (`index.css:2249`) = 12px/600/uppercase/0.08em. `/work-orders` is 12px/600/none/normal, `/expenses` is 12px/400/none/normal. Scope: 4 `labelStyle` objects + ~100 inline `<label>` across 19 files; only 6 files use `.form-group`. **Not half-converted on purpose** — a subset adds a NEW inconsistency axis.
- **`.qa-r91-neverrun.mjs` still unrun — now 8 nights.**
- **NEW, and the concrete next step for the open branch: the OTHER jsonb write sites are unswept.** Tonight's fix covered the two `::jsonb ||` merge sites. Five sites use plain assignment and were out of scope: `contractService.js:114`/`:212` (`content`), `dripService.js:93` (`trigger_config`), `invoiceService.js:117` (`line_items`), `workOrderService.js:377` (`line_items`), `roofMeasurementService.js:344` (`data`). **Does each 400 on a non-object body value, or does it `JSON.stringify` whatever arrives?** A `line_items` stored as a scalar breaks every `.map()` consumer — the Run 68 white-screen shape. Cheap: one PATCH per site + a consumer grep.
- Deliberately not changed, reasoning recorded: the 5 `.nav-link` `!important` pairs are **value-identical no-ops** (deliberate UA-chrome suppression; the in-source comment at `:304-308` shows the author understood the mechanism); the 3 `TasksView` shorthand hits are inside the `isMobile` branch and mobile is paused; `stormHistoryService.js:49` carries the prototype-lookup pattern fixed elsewhere but is called only with hardcoded values and is **not request-reachable**.
- Housekeeping, developer call: 336 QA screenshots committed to the repo root; dead code `quickFilters`/`applyQuickFilter` (`LeadList.jsx:62`, `:346`) and dead CSS `.stat-card:hover .stat-card__icon img` (`index.css:959`); stray 0-byte `server/=`.

### Coverage gaps this run
- **18 of 272 route patterns not executed — BY DESIGN.** Excluded as side-effecting: `trigger-import` (real bulk property import), `geocode` (costs money), outbound delivery (`send|email|sms|webhook`), auth state (`login` is rate-limited at ~10 attempts). **Five ADDED to the exclusion list this run** — `score-all`, `correct-all`, `mark-all-read`, `/complete`, `alerts/test` — after pass 1 proved each mutates real rows.
- ⚠️ **TESTER ERROR — the sweep drove two routes that mutate real rows, both 200.** `POST /api/crm/leads/score-all` re-scored **all 13 leads** (a bulk write the charter forbids; not restorable but **not corrupt** — scores are deterministically recomputed, capped at 100/call, stale-only >24h). `PATCH /api/crm/work-orders/:id/complete` with an **empty body** marked a live work order completed; **reverted** to `status='pending'`, `completed_at=NULL`, `updated_at=created_at` (the seed pattern: untouched rows have `updated_at == created_at`). WO domain back to 7 pending / 3 completed. **GENERAL RULE: an action route (`/complete`, `/score-all`, `/mark-all-read`) needs NO body, so an empty-body sweep does not neutralize it — it FIRES it.** Empty-body safety reasoning only holds for routes that build a SET clause from the body. `.qa-r100-sweep.mjs`'s SKIP regex is now hardened — **reuse that harness, do not re-derive the exclusion list.**
- **Four routes are structurally unexercisable:** the `counties` and `material_products` tables **do not exist** (`42P01`); `contracts.public_token` and `leads.status_token` **are not columns** (`42703`).
- **`/canvassing` pin drop not completed** — costs a DB write + a paid geocode.
- ⚠️ **s2 wrote NO `/tmp/frontend-test-results.txt` this run** — its full record is in its resume entry instead. **`C:/tmp/frontend-test-results.txt` is the 2026-08-26 file and `%TEMP%/ui-audit-results.txt` is the 2026-08-25 file. STALE. Do not read either as current.** Tonight's artifacts are `C:/tmp/api-test-results.txt` and `C:/tmp/ui-audit-results.txt` **only**. Git Bash `/tmp` resolves to `C:/Users/brand/AppData/Local/Temp`, which is **NOT** `C:/tmp`; s1 wrote to both this run precisely because that trap cost two prior nights.
- **`add652c`'s more severe claim is NOT reproducible here.** The commit message says the Add Payment Method button is *permanently* dead when `VITE_STRIPE_PUBLISHABLE_KEY` is absent. **That key IS set locally**, so only the ~1-frame load transient is observable. The fix is correct either way — recorded rather than laundered into a pass.
- No screenshots by s2 or s3, by choice (336 QA PNGs already in the repo root; computed-style + network-param evidence is stronger). s4 took 3 to `C:/tmp`, outside the repo.

### ⚠️ TWO NEW SELECTOR-SCOPE TRAPS (s4, both cost real time)
1. **There is NO `<main>` element — the root is `.main-content`.** A `|| document.body` fallback silently measured the **global Cmd-K palette** and faked a clean empty-state pass on **8 routes**.
2. **`.main-content` DOESN'T EXIST at 375px** on routes with an `isMobile` branch, so a scoped text probe reads **0 characters** and looks like a blank mobile screen when **34–84 nodes are actually painted**.

### Verified non-bugs — do not re-file
- **`POST /api/notifications/mark-all-read` returning 200 while 12 notifications stay unread is CORRECT** — `markAllRead(userId)` is scoped by **`user_id`, not tenant** (`notificationService.js:97`); the 12 belong to 3 other users.
- **`PATCH`/`PUT` with a DEAD uuid returning 200 at `/api/alerts/config`, `/api/crm/tenant-settings`, `/api/materials/credentials`, `/api/onboarding/org` is CORRECT** — these are **tenant singletons with no `:id` in the path**, so the dead id was never substituted.
- **`PATCH` with an empty or unknown-field-only body returning 200 is CORRECT** — both `updateLead` implementations short-circuit `if (setClauses.length === 0) return getLead(...)`, a pure READ.
- **`allowedFields` vs real DB columns: 12 whitelists, 0 mismatches** (self-test 2/2 CAUGHT). **Run 99's item #2 is CLOSED.**
- **3 of the 5 raw `:disabled` "BUG" hits died on a SOURCE READ** — `MaterialsView.jsx:703` sits inside a `{cart.length > 0 && …}` gate; `ImportLeadsModal.jsx:310`'s term is implied by an upstream `filter`; `RoofDrawingTool.jsx:834` substitutes a grey background. **A control can have a LEGITIMATE non-opacity disabled treatment — check for a substitute cue before filing.**
- **Two thirds of s2's first pass was harness error**, in the documented shape: 12 of 52 hits were `.glass` misattributed from `.glass[class*="card"]:not(.stat-card)`, and most of the rest were `cond ? 'x' : undefined` — **React omits an `undefined` property entirely, so an inline style only kills a state when it is UNCONDITIONAL.**
- Only console errors all night: the documented `/api/admin/overview` 403 and the token-refresh 401s.

### Notes for the next run
- ⚠️ **SEVENTH CONSECUTIVE RUN whose defect is "a value silently loses its expected type or precedence, and the failure is invisible at the call site."** Six were CSS-cascade shaped; tonight's backend bug is the **DB-layer twin**. **Triage this shape FIRST.**
- **The CSS-cascade branch is CONVERGED** — specificity, source order, inline-beats-stylesheet, `!important`-beats-inline, `!important`-beats-own-state, shorthand resets and duplicate declarations are all swept clean. **DO NOT RE-SPEND THERE.** The open branch is the DB/type one; the unswept jsonb write sites above are its next concrete step.
- **Assign `automationEngine.js:75-77` to a named stage.** Eight runs of "it's someone else's stage" is the actual failure mode here.
- **Read each stage's JSON `terminal_reason`, not its self-assessment** — s1 claimed it did not cap when it did.
- Drift baseline for the next run: the `docs: QA report 2026-08-28` commit (head after this entry).

---

## QA Run: 2026-08-29

**Branch** `feat/financing` · **Baseline** `1e5c194` · **Head** `d41e868` · **Build** PASS (vite 8.17s, exit 0) · **Net DB writes** 0

### Stage completion
| Stage | Run | Terminal reason | Turns |
|---|---|---|---|
| s1 api-test | 105 | **`max_turns` (50)** | 51 |
| s2 frontend-test | 105 | `end_turn` | 69 |
| s3 ui-audit | 106 | `end_turn` | (complete) |
| s4 verify | 107 | **`max_turns` (40)** | 41 |
| s5 report | 108 | `end_turn` | this entry |

⚠️ **Two of five stages capped. s1 has now capped three runs running; s4 capped for the first time and produced NO verification report at all.** Run numbering collided — s1 and s2 both self-labelled Run 105.

### Test Results
- Pages tested: **19 routes** (18 by s2, 19 by s3 incl. `/alerts`) + **15 Settings tabs** + **6 modals/slide-overs** opened and probed
- API endpoints tested: **198 distinct endpoints / 248 endpoint+method pairs** of a 272-route inventory; **~498 requests**, **0 5xx**
- Bugs found: **3**
- Bugs fixed: **3**
- UI inconsistencies found: **1 family — 13 rule groups / 27 dead CSS declarations, one root cause**
- UI inconsistencies fixed: **1 family (all 27)**
- Commits: **4** — 3 fixes (`f9bb21e`, `f8354cf`, `d41e868`), 1 harness carry (`e34c27a`)

### Fixes Made
- `f9bb21e` — **the contracts `content` JSONB column accepted a scalar and white-screened the contract page, including the PUBLIC customer-facing one.** All four write paths (`POST /crm/contracts`, `PATCH /crm/contracts/:id`, `POST` + `PATCH /crm/contracts/templates`) wrote the column with a bare `JSON.stringify` and **no shape guard**, so `content: "oops"` and `content: {sections: "oops"}` were both accepted and persisted with a **200**. ⚠️ **The server PDF path survives only by accident** (`Array.isArray(rawContent.sections) ? … : []`); the client does not — `parseSections` returned `content.sections` on truthiness alone and `PublicContract.jsx:130`, `ContractsView.jsx:601` and `:656` all call `sections.map()` on the result. Fixed by guarding the write path the way invoices/work-orders already guard `line_items`, **and** tightening both `parseSections` copies to `Array.isArray` — the Run 68 lesson is that a write-path guard cannot clean rows that already hold junk. Verified: 6 bad shapes → 400, real payload still 200s, build PASS 11.82s.
- `f8354cf` — **a list response missing its array key white-screened the ENTIRE SPA.** Found by a response-shape sweep never run here: 11 routes × 3 modes (500 / body `{}` / top-level arrays nulled), stubbing the primary GET with `page.route` + `route.fetch`. **32 of 33 probes degraded gracefully; `/contracts` blanked on 2 of 3** (body length 0, no `<h1>`, `contracts.filter is not a function`). Root cause `ContractsView.jsx:55` — `res.data.contracts || res.data || []`; **the `|| res.data` fallback turns a missing key into the WHOLE response object** — truthy, not an array. ⚠️ **Because the app has no error boundary the sidebar and header go too.** 7 sites shared the idiom; the app's own convention at **6 other** list setters is `res.data.X || []`, which cannot produce a non-array — **that asymmetry is exactly why `/contracts` was the only one of 11 routes to crash.** Fixed by keeping the bare-array fallback but requiring it to BE an array. **Reachability: latent, not live** — all 7 endpoints currently return their key; fixed anyway per the standing Run 68 rule. Verified: repro `len 0 → 432` with the proper empty state; 5 touched views byte-identical (626/817/1596/632/638). Build PASS 7.98s.
- `e34c27a` — carried s1's two finished-but-uncommitted harnesses (`.qa-r105-jsonb.mjs`, `.qa-r105-typed.mjs`) after it capped.

### UI Consistency Fixes
- `d41e868` — **27 FullCalendar style rules were dead because of LAYER ORDER — a cascade mechanism no prior run had modelled.** `client/src/index.css` is wrapped in `@layer base` (line 6) while **FullCalendar v6 self-injects its stylesheet UNLAYERED**, and ⚠️ **an unlayered NORMAL declaration beats every layered one REGARDLESS OF SPECIFICITY.** So `.calendar-view .fc …` overrides at specificity up to **402** were losing to FC's specificity-**100** rules. Measured on `/calendar`, each confirmed on EVERY matching element: title 24.5px not 18px (1/1); day-header cushion 2px not 10px (7/7); Today button transparent with no border (1/1); chevrons 19.5px not 18px (1/1); `.fc-button` block not inline-flex (7/7); grid edge borders still painting (43/43, 7/7); `.fc-button-group .fc-button` border (6/6); header border colour (7/7); day-number padding 4px not 0 (42/42). ⚠️ **THE AUTHOR HAD ALREADY HIT THIS ONCE** and patched only `.fc-button-primary` *typography* with `!important`, documenting the mechanism at `index.css:3780`. **Fixed by closing `@layer base` around the FC block, NOT by `!important`** — which would have inverted the cascade against that block's own non-important `:hover` / `.fc-button-group` / `.fc-day-other` overrides and killed those instead, **manufacturing the exact Run 102 defect shape the audit was there to find.** Verified: rendered values match source; intended overrides preserved; `:hover` proven alive with a real pointer; **built bundle grepped** (FC block sits past the layer close, nothing reordered); 19-route sweep 18/19 byte-identical, `/calendar` unchanged. Build PASS 8.01s.
- **BOTH of Run 102's named gaps CLOSED as EMPTY.** (a) Cross-selector `!important` kill: 1,839 static candidate pairs → **28 real co-matching pairs** via runtime `el.matches`; 3 confirmed kills, **all deliberate Tailwind `!` utilities**. (b) CHECK C/D **inside modal interiors** (never swept — both prior runs measured at rest only): 6 overlays, **1,439 inline declarations probed, 0 hits**. ⚠️ **The CSS cascade family is now swept in every direction — specificity, source order, inline, `!important`, and layer — and has CONVERGED. DO NOT RE-SPEND THERE.**
- **Audits 1–7 regression-clean across 19 routes in one browser call.** Icons: 0 foreign SVGs, 0 `fa-*`/`material-icons`/`lucide`, every app svg `viewBox="0 0 24 24"`. Buttons: every radius/height signature maps to a documented family. Headers: exactly one `<h1>` on 19/19 with correct titles. Nav: 18 links + 18 icons on 19/19, `.is-active` = 1 on 18/19 (`/alerts` = documented orphan). Forms: **0 native `<select>`, 0 native `<input type=date>`** source and runtime. Spacing: 0 horizontal overflow 19/19. Modals: 0 overlays at rest 19/19.

### Verification (s4, Run 107) — NONE
⚠️ **s4 capped at turn 41 of 40 and produced no report and no artifact.** Tonight's three fixes rest on **each stage's own self-verification** — documented and evidence-backed in every case, but **no independent second pass ran.** s4's harness `server/src/routes/.qa-r107-s4-verify-contracts.mjs` (re-verifies `f9bb21e` across all four contract write paths, using identity writes for the happy path) is **carried into the tree unvalidated — it has never been executed to completion.** **TOP ITEM FOR THE NEXT VERIFY STAGE.**

### Known Issues Remaining
- ⚠️ **`automationEngine.js:75-77` `lead_priority` enum bug — NOW 10 RUNS OLD.** Has a written repro; keeps being skipped because **NO STAGE IS EXPLICITLY ASSIGNED IT.** Not frontend-reproducible, so the frontend stages correctly decline it; the api-test stage has never been told to take it. **NAME IT IN THE api-test PROMPT.**
- **s1's `.qa-r105-jsonb.mjs` and `.qa-r105-typed.mjs` are committed but their RESULTS have still never been read** (s1 capped before reporting, 2nd run in a row for these two). They are write probes against the production Neon DB, so s2 and s3 both correctly declined to run them out of scope.
- **`EstimatesView.jsx:1318` — measured, deliberately NOT fixed.** `/estimates` → Edit white-screens with `templates.map is not a function` when the server returns a truthy non-array inside the `templates` key. ⚠️ **This is a MUCH WEAKER claim than `f8354cf`'s and the distinction is the point:** `:1318` uses the app's *correct* `|| []` idiom and only breaks if the server actively lies about the type; `estimates.js:96-99` returns a query rowset, always an array. Guarding it means `Array.isArray`-ing every `.map()` in the client — the refactor the charter forbids. **Recorded so it is not re-chased as a fresh finding.**
- ⚠️ **THE APP HAS NO ERROR BOUNDARY ANYWHERE.** Any render throw blanks the whole SPA including the sidebar. **It is the amplifier behind BOTH of tonight's frontend-visible bugs and Run 68's.** Adding one is an ENHANCEMENT (out of QA charter) but it is the single highest-leverage robustness item in the backlog — **put it to the developer.**
- **`qa_options_probe` custom field STILL live in the production DB** (tenant `waterloo`, created 2026-08-05, label "QA Options Probe"). The hygiene sweep matches `'QA-R9%'`, which this label does not match, so it escapes cleanup every night. Confirmed safe to delete (0 leads carry a value). **Needs one `DELETE` + developer go-ahead — not QA's call.**
- **Form-label drift — DEFERRED, developer decision, 6th run.** Canonical `.form-group label` (`index.css:2249`) = 12px/600/uppercase/0.08em; `/work-orders` = 12px/600/none/normal, `/expenses` = 12px/400/none/normal. Scope: 4 `labelStyle` objects + ~100 inline `<label>` across 19 files; only 6 files use `.form-group`. **Not half-converted on purpose** — a subset adds a NEW inconsistency axis.
- **`.qa-r91-neverrun.mjs` still unrun — now 10 nights.**
- Housekeeping, developer call: 336 QA screenshots committed to the repo root; dead code `quickFilters`/`applyQuickFilter` (`LeadList.jsx:62`, `:346`); dead CSS `.stat-card:hover .stat-card__icon img` (`index.css:959`); stray 0-byte `server/=`.

### Coverage gaps this run
- ⚠️ **NO INDEPENDENT VERIFICATION PASS RAN** (s4 capped). See above.
- ⚠️ **The detail-level type sweep only PARTLY ran — 1 of 4 cases actually exercised.** `/work-orders` and `/invoices` **issue no detail GET at all** (the panel renders from the row object the list already holds), so there is nothing to stub; and the work-order expander selector `.glass, [class*="card"], tbody tr` filtered to length>20 **matched the OUTERMOST page wrapper first**, so the click hit the page container, not a card. **Recorded as NOT EXERCISED, not as passing.** Next run: identify the real card expander FIRST, then stub. (The documented handle for the milestone toggle is `button[aria-label="Mark <name> complete"]`; the expander is not yet identified.)
- **18 of 272 route patterns not executed — BY DESIGN.** Excluded as side-effecting: `trigger-import` (real bulk property import), `geocode` (costs money), outbound delivery (`send|email|sms|webhook`), auth state (`login` is rate-limited), plus the action routes `score-all`, `correct-all`, `mark-all-read`, `/complete`, `alerts/test` added to the exclusion list last run.
- **Four routes remain structurally unexercisable:** the `counties` and `material_products` tables **do not exist** (`42P01`); `contracts.public_token` and `leads.status_token` **are not columns** (`42703`).
- **`/canvassing` pin drop not completed** — costs a DB write + a paid geocode.
- **No screenshots taken by any stage this run, by choice** (336 QA PNGs already in the repo root; computed-style + network-param evidence is stronger for every check performed).

### New tester traps recorded this run
- ⚠️ **TEST EVERY MATCHING ELEMENT, NOT `els[0]`.** `.fc-daygrid-day-number` `color` read as a dead declaration — but the probe tested only `els[0]`, which is a `.fc-day-other` cell that `index.css:3945` **deliberately** dims. Dead on 12/42, alive on the other 30. **A partial hit is a legitimate modifier override; only 100% is a real kill.**
- ⚠️ **KILL `transition` BEFORE READING COMPUTED VALUES.** The first colour sweep produced `oklab(...)` values that looked like corruption — they were **mid-transition reads** (`.fc-button` carries `transition: all 0.15s`). Re-measured with transitions killed, every value landed exactly on its declared target.
- **`/src/index.css?direct` gets the REAL compiled cascade into the page** — this sidesteps the `document.styleSheets`-returns-`[]` trap. `import`/`require` do NOT work inside `browser_run_code_unsafe`.
- **THE METHOD TO REUSE for any future precedence mechanism:** re-assert the top-specificity app declaration inline with `!important` and see whether the computed value moves. Empirical, no cascade theory required, and it generalises — it is what caught a mechanism (layer order) that five prior cascade runs had no model for.
- **THE ESTIMATE BUILDER AND REVIEW MODE RENDER OUTSIDE `.main-content`** — a probe scoped there returns NO_ROOT. Third instance of the Run 103 trap. Scope by EXCLUDING chrome instead.
- **A create-button regex matching new/add/create MATCHES FORM SUBMIT BUTTONS, not just openers.** "Add Payment Method" is a submit inside a mounted Stripe form. Before filing "the create button is dead", check whether the control sits inside a `<form>` — an opener never does.
- **DO NOT INDEX A BUTTON LIST CAPTURED BEFORE THE CLICK.** Re-find by innerText immediately before each click, tag with a `data-*` attr, and re-enter the flow between probes.
- **STUB-BASED SHAPE TESTING IS THE RIGHT TOOL for the "value loses its type" family on the frontend** — `page.route` + `route.fetch` + mutate + fulfill drives the real component tree end-to-end with ZERO DB writes, and it is the only way to reach a stored shape the database does not currently hold. Always guard the handler with try/catch and fall back to `route.continue()`.

### Notes for the next run
- ⚠️ **EIGHTH CONSECUTIVE RUN whose defect is "a value silently loses its expected type or precedence, and the failure is invisible at the call site."** Tonight produced all three variants at once: a DB-layer type loss (`f9bb21e`), a client-layer type loss (`f8354cf`), and a CSS-layer precedence loss (`d41e868`). **Triage this shape FIRST.**
- **The CSS cascade branch is CONVERGED — do not re-spend there.** The one measured extension left is **mapbox-gl**, the OTHER unlayered vendor stylesheet: the same layer inversion must apply to every `.mapboxgl-*` override. ⚠️ **Map code is off-limits to edit — MEASURE AND REPORT ONLY.** After that, component-class-vs-Tailwind-`utilities`-layer.
- **The open branch is the DB/type one**, and its concrete next step is the detail-level sweep that only partly ran tonight.
- **Read each stage's JSON `terminal_reason`, not its self-assessment.**
- ⚠️ **Two stages capping in one night is now a pipeline problem, not a stage problem.** s1 has capped 3 runs running and its analysis has been lost all three times; consider raising its turn budget or narrowing its charter.
...
**Runs 122–125 · branch `feat/financing` · baseline `7662ce8` → HEAD `cc13c6d` · final build exit 0, 7.99s**

**Three of five stages hit their turn cap (s1, s3, s4) and wrote no artifact.** `/tmp/api-test-results.txt` and `/tmp/ui-audit-results.txt` on disk are dated 2026-09-02 (Runs 117/119) and are NOT this run's output. The API and UI sections were reconstructed from tonight's raw harness JSON in `C:/tmp/`, the commit record, and one re-run of the write-free half of s4's lost verification.

| Stage | Terminal | API time | Cost | Artifact |
|---|---|---|---|---|
| s1 api-test (122) | **capped** | 691s | $5.66 | none |
| s2 frontend-test (122) | **completed** | 745s | $6.23 | written |
| s3 ui-audit (123) | **capped** | 385s | $5.34 | none |
| s4 verify (124) | **capped** | 522s | $4.42 | none |

### Test Results
- Pages tested: **18** (functionally driven in a browser, not just rendered)
- API endpoints tested: **200 live** of a **272-route / 36-file** inventory (132 GET + 34 POST + 34 POST no-body); **62** POST guard sites + **23** falsy-guard sites analysed statically
- Bugs found: **5**
- Bugs fixed: **4** (the 5th is incomplete and currently broken — see below)
- UI inconsistencies found: **0 measured** — audits 1–7 did not run
- UI inconsistencies fixed: **0**
- Regressions: **0** · DB: **net zero**, tenant-scoped counts byte-identical

### Fixes Made
- **`43e843e`** `fix(api)` — an explicit `null` for a whitelisted field on a `NOT NULL` column returned **500**, not 400. `23502 not_null_violation` was missing from `PG_BAD_INPUT_CODES`. Now maps to 400 and names the column via `err.column`. 11 pairs across 5 tables. (`errorHandler.js`, +15)
- **`824ae24`** `fix(api)` — `line_items: null` **silently wiped a NOT NULL jsonb column behind a 200**, leaving `subtotal`/`total` stale, because `JSON.stringify(null)` stores a *jsonb null* that satisfies `NOT NULL`. Null test added to the array and object branches, exempting `estimates.deposit` (the UI sends null to clear it). **Written by s1, left UNCOMMITTED when s1 capped at 51/50; recovered, verified 8/8 both directions, and committed by s2.** (3 route files, +27/−6)
- **`282f51f`** `fix(ui)` — `/estimates` and `/leads` rendered empty-*database* copy when a filter matched zero rows. `EstimatesView` had no guard; `LeadList` *had* one that omitted 2 of its 6 server-side filters (`scoreFilter`→`min_score`, `repFilter`→`assigned_rep_id`). (2 components, +7/−3)
- **`cc13c6d`** `fix(ui)` — a **failed** list load rendered the same empty-database copy with zeroed stat headers and no toast, banner or any other sign of failure. Six views swallowed the fetch error. Proved live with a 500 stub on each list GET (`/contracts` 4 rows → "0 contracts"; `/invoices` 14 → "$0 TOTAL INVOICED"; `/estimates` 16 → "0 estimates"; `/leads` 13 → "0 leads"). `loadError` flag checked ahead of the filter branch. (6 components, +60/−28)

Fixes 3 and 4 are the same family — empty-state copy asserting something about the database the current context does not support. Three runs have now found instances (`68c968a`, `282f51f`, `cc13c6d`); a 7th instance is open below.

### UI Consistency Fixes
- **NONE — and, unlike the previous 18 runs, this is not a clean result.** s3 capped before running audits 1–7. Icons, buttons, toolbars/headers, sidebar/nav, spacing and modals are **entirely unmeasured tonight**. Run 119's "0 defects for the 18th consecutive run" is the last real measurement and was NOT carried forward as if it were tonight's.
- **Audit 5 (forms) is the one exception and it passed:** s2 confirmed **0 native `<select>` and 0 native date/time inputs across all 15 Settings tabs** — `CustomSelect`/`DatePicker` compliance holds.

### Independent re-verification recovered (s5)
s4 wrote `server/.qa-r124-s4-serverfix.mjs` to verify both server fixes, then capped — its output went to stdout and was lost. s5 re-ran **only the write-free half** (every expect-400 case fails before the UPDATE), asserting counts + `max(updated_at)` before and after: **pass 13 / fail 0 / skip 2 / unverifiable 2, nothing written.** Both server fixes are now independently confirmed against the running server.

### Known Issues Remaining
- 🔴 **NEW AND OPEN — `client/src/components/AutomationSettings.jsx` is BROKEN in the working tree, uncommitted, and `vite build` does not catch it.** s4 found the **7th instance** of the empty-state family (Settings → Automations swallows its load error) and began applying the `cc13c6d` fix at 05:50:19, then capped mid-edit. It calls `setLoadError(false)` at :82 and `setLoadError(true)` at :85 but **never declares `loadError`** (7 `useState` calls, none of them it); :253 still has the bare `automations.length === 0`. Build passes because it is a runtime `ReferenceError`, not a compile error — the `try` throws at :82, the `catch` throws again at :85, and `setLoading(false)` never runs. **Left as-is deliberately by s5** (reporting charter; completing another stage's unverified edit is how regressions get committed). Needs a revert or 2 lines. **Decide before the next run — the next `git status` sweep will otherwise treat it as recovered work and may commit it broken.**
- ⚠️ **No error boundary anywhere in the SPA** — the amplifier behind five past defects including the one above. Developer's call.
- **64 generic error toasts discard the server's message** vs 22 that surface it. Re-confirmed tonight on `/invoices` and `/contracts` empty-form submit (400 arrives, row correctly not created, user sees only "Failed to save invoice").
- **Form-label drift — 7 treatments, ~100 inline labels. 12th run deferred.**
- **`/alerts` orphan route** — design decision. **`22007`/`22008` still leak the raw pg message** — pre-existing, untouched by `43e843e`. **`EstimatesView.jsx:1318`** — deliberately not fixed.
- **DB junk rows awaiting go-ahead:** subcontractor `{"$eq":1}`, territory `12345`, `qa_options_probe` custom field.
- **361 QA screenshots in the repo root** — this run added **0**. **`server/.qa-r91-neverrun.mjs` — never run, 16 nights.**
- **10 untracked `.qa-*.mjs` harnesses** from tonight (9 × r122, 1 × r124) + 5 `claude-overnight-*.json` in the repo root; s5's commit is scoped to `OVERNIGHT-REPORT.md` and `docs/`.

### Coverage gaps this run
- ⚠️ **The UI consistency audit did not run at all** — second time in three nights the s3 charter has gone unmeasured.
- ⚠️ **s1 and s4 wrote no artifact.** Both were reconstructed by s5 from raw JSON and a re-run — sound, but it consumed most of s5's budget. **Require every stage to write its artifact BEFORE spending its remaining budget.** The one stage that did (s2) needed no reconstruction.
- **`PATCH /api/crm/tasks/:id` still unexercisable** — `tasks` holds **0 rows**. s1 created a fixture at ~05:07, probed it, removed it (net-zero), so 7 whitelisted fields remain the last unprobed typed columns in the CRM core. **A durable fixture unblocks it.**
- 🆕 **`contract_templates.name`/`.type` unverifiable — and possibly a real user-facing bug.** All 4 template rows have `tenant_id = NULL` / `is_default = true`; the waterloo tenant owns **zero**. `updateTemplate` filters `WHERE id=$2 AND tenant_id=$1`, so a PATCH 404s ("Template not found or not editable") before reaching the null guard. **Candidate defect, unverified: Settings → Contracts lists 4 default templates — if the UI offers Edit on them, that click 404s for every tenant. Next s2 should drive that path.**
- **14 `GET` routes returned 200 with expected response fields missing** — captured in `qa-r122-sweep.json`, not yet triaged. Narrows the standing "GET response shapes unasserted" gap from 132 unknowns to 14 known disagreements.
- **28 `POST` routes skipped** as side-effecting; **POST create-path type confusion still largely unmeasured** (`createX` often bypasses `allowedFields` and destructures directly — `createWorkOrder` does).
- **7 tenant-singleton write routes uncovered** (no `:id`): `/api/alerts/config`, `/api/crm/tenant-settings`, `/api/materials/credentials`, `/api/onboarding/org`, `/api/roof-measurement/config`, `/api/skip-trace/config`, `/api/notifications/preferences`.
- **`territories` table still does not exist** — 4 routes structurally unexercisable.
- **Not swept:** `/content-studio` (unbuilt, not broken), `/canvassing` pin drop (DB write + paid geocode), `/pipeline` drag, `/leads/:id` activity modal + score popup.
- **No screenshots taken, by choice.**

### DB hygiene — the "net zero" claim, scoped honestly
- Tenant-scoped counts for `791bb51d` (waterloo), whole night: `leads 13 · tasks 0 · estimates 16 · invoices 14 · work_orders 10 · contracts 4 · expenses 2 · subcontractors 64` — **byte-identical** to s2's pre-run snapshot.
- s5's recovery re-run asserted counts + `max(updated_at)` before/after: **identical, nothing written.**
- Only genuine side effect all night: a `tenants.updated_at` bump at 05:05:40 from s1's write probe.
- 🚫 **TESTER-ERROR TRAP, NEWLY RECORDED.** An **unscoped** `select count(*) from estimates` returns **17**, not 16 — the 17th row belongs to a different tenant. This looked like real overnight drift for several minutes. **Always scope count queries by `tenant_id` in a multi-tenant DB before reporting drift.**

### Notes for the next run
- 🔑 **DECIDE ON `AutomationSettings.jsx` FIRST** — it is the only thing in the tree that is actively broken.
- ⚠️ **Stage capping is still the dominant failure mode, not any individual defect.** s1, s3 and s4 all capped; between them they account for every coverage gap above. Tonight's defect count reflects how much testing actually ran, not how sound the app is. **Raise the s1/s3/s4 turn budgets or narrow their charters.**
- 🔑 **Batching is what lets a stage finish.** s2 completed by collapsing work into few, wide calls; the three capped stages did not. Never one call per route.
- **`git status` as a first action recovered a complete, uncommitted fix for the 6th separate night** (`824ae24`). Keep it first — and note it cuts both ways: tonight the same sweep would have picked up a *broken* edit.
- **Read each stage's JSON `stop_reason`, not its self-assessment**, and **always date-check `C:/tmp/*.txt` before treating an artifact as tonight's.** Both rules paid again: the api and ui-audit files were a day stale and reading either as tonight's would have fabricated two whole sections.
- **Harness traps re-confirmed:** the Bash tool's **cwd persists** (bit s5 once — a `.mjs` importing `./src/db/pool.js` must be run from `server/`, and a later relative path then fails); **`server/src/db/pool.js` has a DEFAULT export, not a named `pool`** — `import pool from` , not `import { pool } from`.
- **Suggested next targets** — **s1:** a durable `tasks` fixture, then the 7 tenant-singleton routes, then POST create paths. **s2:** the contract-template Edit path (candidate 404 above). **s3:** actually run audits 1–7; they have now gone unmeasured twice in three nights.
- Drift baseline for the next run: the `docs: QA report 2026-09-03` commit (head after this entry).

---

## QA Run: 2026-09-04

**Branch `feat/financing` · baseline `a996c76` (`pre-overnight-20260904`) → HEAD `1733a1c`**
Stages: s1 api-test · s2 frontend-test · s3 ui-audit · s4 verify · s5 report.
**s1, s3 and s4 all hit `error_max_turns`** (51/50, 61/60, 41/40). Only s2 completed.
s3 capped *after* writing its artifact and committing its fix; **s1 and s4 wrote no
artifact at all** — s1's results were reconstructed by s5 from the JSON its harnesses
left in `C:/tmp`, and **s4 left no verdict of any kind.**
Stage cost: **$23.63** (s1 $5.27 · s2 $8.68 · s3 $5.69 · s4 $3.99).

> Run-number note: s2 called tonight *Run 123*, s3 called it *Run 124*. Harness filenames
> inherit the split (`.qa-r123-*`, `.qa-r124-*`). Resynchronise before the next run.

### Test Results
- Pages tested: **19/19 top-level routes** + **15/15 `/settings?tab=` sub-panels** (a surface never swept before) + `/leads/:id` slide-over (8 controls) + **13 create controls** across 8 routes and 3 Settings panels
- API endpoints tested: **170 probes** — **132 GET** (every GET in the inventory: 113×200, 5×403 expected-admin, 4×400 expected-missing-param, 10 skipped for absent tables, **0×5xx**) + **38 collection-level POST/PUT** empty-body probes (35×400 correct, 3×200 unclassified PUT singletons)
- Route inventory: **272 routes / 36 files** — GET 132 · POST 88 · PATCH 26 · PUT 8 · DELETE 18 (unchanged since Run 114)
- Client source files statically swept: **76** (new undeclared-setter check)
- Bugs found: **4**
- Bugs fixed: **4** (2 commits)
- UI inconsistencies found: **0** — audits 1–7 all converged
- UI inconsistencies fixed: **0** (none to fix)
- Regressions: **0** · Build: **PASS** (s2 12.20s · s3 8.01s · s5 8.12s) · DB: **net zero**, tenant-scoped counts byte-identical
- Screenshots added to repo: **0**

### Fixes Made
- **`71d06dc`** `fix(ui)` — **`/subcontractors` told a tenant holding 63 subcontractors to "Add Your First Subcontractor"** whenever a filter matched zero rows. All three filters (search, specialty, status) go server-side and refetch. Reachable two ways, both proved live pre-fix. Fixed with the ternary idiom from five sibling views; `status` defaults to `'active'`, so only a *change* from that default counts as filtering — otherwise a genuinely empty tenant loses its onboarding CTA. Verified both directions + an empty-list stub, zero DB writes. (`SubcontractorsView.jsx`, +14/−4)
- **`092d37a`** `fix(ui)` — **Settings → Automations was a DEAD PANEL.** The whole Workflow Automations feature was unreachable; the panel sat on "Loading automations..." forever with `PAGEERROR: setLoadError is not defined` ×2 per load. `setLoadError()` was called at :82 and :85 with **no `useState` behind it** — the `try` threw, the `catch` threw again with nothing to catch it, and `setLoading(false)` at :87 never ran. Same commit fixed two wrong-copy siblings: a *failed* automations load claimed "No automation rules yet. Create one to get started.", and `DripSequences.jsx` had the identical `catch { /* ignore */ }` + bare `length === 0` shape. Fixed by **completing** the abandoned edit, not reverting it, using the `cc13c6d` idiom. Verified both directions on both panels with a stubbed 500. (`AutomationSettings.jsx`, `DripSequences.jsx`, +13/−3)
- Non-fix commits: **`130eff6`** (2 read-only s2 harnesses), **`1733a1c`** (`server/.qa-r124-undeclared.mjs`).

🔑 **ROOT CAUSE OF THE DEAD PANEL — last night's warning came true within 24 hours.**
`a996c76`, **tonight's own 05:00 checkpoint**, swept in the half-applied edit that
2026-09-03's s4 abandoned when it capped mid-edit. Last night's s5 deliberately left it
uncommitted and broken and wrote: *"the next `git status` sweep will otherwise treat it as
recovered work and may commit it broken."* That is exactly what happened. **The "diff the
tree for a capped stage's uncommitted FIXES" rule cuts both ways — a capped stage can also
leave a broken edit, so classify what you find before committing it.**

Three blind spots then lined up, all now standing facts:
1. **`vite build` cannot catch it** — an undeclared identifier is a runtime `ReferenceError`, not a bundling error. **Three stages passed a green build tonight over a dead panel.**
2. **There is NO ESLINT in this project** — no config, no script, no dependency. `no-undef` is exactly the rule for this. Reported, not added (tooling is outside the QA charter).
3. **The 19-route sweep loads `/settings` on its DEFAULT tab only.** All 15 panels are `?tab=` sub-surfaces and no audit had ever entered them looking for **render health** — only for tab-chrome consistency. R122 measured 15/15 tabs *navigate*, and they do; the panel behind one was dead.

### UI Consistency Fixes
- **NONE NEEDED — audits 1–7 all measured CLEAN, and unlike 2026-09-03 this is a real measurement, not a gap.** Icons: 0 foreign classes on 19 routes + 15 panels (the only odd viewBoxes are recharts surfaces on `/reports`). Buttons: 0 unnamed, uniform `oklch(0.72 0.19 250)` primaries, house radius set. Headers: **exactly 56px on 19/19 — 20th consecutive convergence.** Sidebar: 22/22 nav buttons, one `.is-active` on 17/19 with both exceptions documented. Forms: **0 native `<select>`, 0 native date inputs** across routes *and* Settings panels. Spacing: 0 overflow, 0 stray overlays. Modals: 13/13 opens clean, dimensions and animations consistent.
- New documented non-defect: `__PrivateStripeElement-input` on `?tab=billing` lacks `.form-input` because **Stripe's own iframe injects it** — vendor-owned, not an app control.
- Two cosmetic readings deliberately NOT "fixed": `/materials`' 136 identical per-product Add buttons (one group, not 136 outliers) and `/`'s `border-radius: 3.35544e+07px` (Chrome's clamp of a huge radius; paints identically to the 999px pills). **Do not "fix" either.**

### New reusable guard
**`server/.qa-r124-undeclared.mjs`** (`1733a1c`, read-only) generalises tonight's defect:
every `setXxx()` **call** checked against a declaration in the same file, across all 76
client source files. **Before fix: 1 hit. After: 0 app-wide.** Only remaining match is
`map.setMapTypeId()` in `StormMap.jsx` — a Google Maps method, false positive, map code
off-limits. **Re-run it whenever a stage caps mid-edit.**

### Non-bugs recorded so they never cost another run
- **Dashboard "No pipeline data yet"** is a pixel-perfect replica of tonight's `/subcontractors` defect shape, and `getFunnel()` genuinely **is** filter-scoped — but the endpoint returns all stages with zero counts, so `funnel.length === 0` is **UNREACHABLE**. Measured empty in **0 of 4** filter states. **DO NOT FILE.**
- **TopBar global search** reads as a dead control to any probe scanning `position:fixed` panels — `.search-dropdown` is `position:absolute`. It works fully, Ctrl+K included. **Sixth instance of "check the structure before filing".**
- **A real Playwright pointer click DOES fire FullCalendar `dateClick`** — the documented trap applies only to synthetic `element.click()`. Gap closed.
- **`/leads/:id` has NO tab rail** — the charter's "test every tab" describes a UI that does not exist. It is one slide-over, 8 sections, 21 buttons, all measured clean.
- **`/leads`, `/calendar`, `/materials` have no "New X" button BY DESIGN** — leads come from Import/Export and the storm map, events from clicking a date, orders from the catalog/cart.

### Known Issues Remaining
- 🔴 **NO ESLINT IN THE PROJECT.** New this run and **the highest-leverage item on this list** — tonight's worst defect in ~20 runs is precisely what `no-undef` catches and the build provably cannot.
- 🔴 **62 candidate tenant-scope SQL sites, UNTRIAGED.** s1's new static audit flagged every statement keying on a row id without a `tenant_id` constraint: 24 writes / 38 reads across 24 files (`routes/properties.js` 11 · `services/dripService.js` 9 · `services/crmService.js` 5 · `routes/onboarding.js` 4 · `routes/payments.js` 4 · `services/windDriftService.js` 4). **Raw scan hits, NOT confirmed defects** — several are obviously fine on sight (Stripe webhooks keyed on a Stripe id, ingestion, `seed-dev.js`). s1 capped immediately after writing the file. **This is a multi-tenant isolation question and the highest-value unfinished work of the night.**
- ⚠️ **No error boundary anywhere in the SPA** — the amplifier behind five past defects. Tonight's crash was in an effect, so it "only" froze one panel.
- **64 generic error toasts discard the server's message** (`InvoicesView.jsx:570`).
- **Form-label drift — 7 treatments, ~100 inline labels. 12th run deferred.**
- **Empty-state STYLING is split** — only `TasksView` uses `.empty-state`; 9 other list views inline-style theirs. Refactor candidate, not a defect.
- **`ActivityModal` has no Esc handler** — closes via backdrop + `aria-label="Close"`, so not a trapped modal. Known-and-deferred.
- **`/alerts` orphan route** · **`22007`/`22008` still leak the raw pg message** (deliberate `else` in `errorHandler.js`).
- **`territories` table still does not exist** — 4 routes structurally unexercisable.
- **`contract_templates` candidate defect, still unverified** (carried from 09-03): all 4 rows are `tenant_id = NULL` / `is_default = true`, waterloo owns zero, and `updateTemplate` filters `WHERE id=$2 AND tenant_id=$1` — so **if Settings → Contracts offers Edit on those 4 defaults, that click 404s for every tenant.** s4 was reaching for this when it capped.
- **DB junk rows awaiting go-ahead:** subcontractor `{"$eq":1}`, territory `12345`, `qa_options_probe`.
- **361 QA screenshots at the repo root** (337 tracked) — **this run added 0.** **`server/.qa-r91-neverrun.mjs` — never run, 17 nights.** **8 untracked `.qa-*.mjs`** + 5 `claude-overnight-*.json` in the tree; this commit is scoped to `OVERNIGHT-REPORT.md` and `docs/`.
- **`server/.qa-r124-s4-serverfix.mjs`** was committed by `a996c76` *before this run began* — a stage-4 harness predating its own run.

### Coverage gaps this run
- 🔴 **s4 (verify) produced NO verdict whatsoever.** Capped at 41/40 having written three read-only harnesses and nothing else. Its entire charter — re-verify every fix, edge cases, 375px responsive, browser back/forward — is **unmeasured tonight**. Both fixes *were* verified by the stages that made them (both directions, with stubs), but received no **independent** re-verification.
- 🆕 **s4 was reaching for a genuine blind spot worth inheriting: the three PUBLIC, customer-facing routes `/estimate/:token`, `/contract/:token`, `/status/:token`.** They live **outside the authenticated app shell and have therefore NEVER appeared in any 19-route sweep.** `.qa-r124-s4-tokens.mjs` already pulls real tokens for all three; `.qa-r124-s4-mergefields.mjs` asks whether the `{{merge field}}` vocabulary the contract UI advertises is ever actually substituted **before a customer sees the public contract page**. Neither was run to a recorded result.
- 🔴 **`tasks` fixture — SECOND consecutive night written and not run.** `tasks` = 0 rows, so `/tasks` is testable only empty and `PATCH /api/crm/tasks/:id`'s 7 fields stay the last unprobed typed columns in the CRM core. `.qa-r123-taskfixture.mjs` exists and needs **one command**.
- ⚠️ **3 PUT tenant-singleton routes returned 200 to an empty body and were never classified** — `/api/alerts/config`, `/api/crm/tenant-settings`, `/api/materials/credentials`. **Row counts prove no row was CREATED; they cannot prove no column was MUTATED.** `/api/materials/credentials` came back with `preferred_branch_id: "true"`, which looks like residue from an earlier probe. The other 4 singletons (`/api/onboarding/org`, `/api/roof-measurement/config`, `/api/skip-trace/config`, `/api/notifications/preferences`) were not probed at all.
- **POST create-path type confusion still largely unmeasured** — 88 POST routes; `createX` often bypasses `allowedFields` and destructures directly (`createWorkOrder` does).
- **GET response SHAPES still unasserted** — 132 routes return 200, nothing checks the payload against what the client reads; R122's 14 known disagreements remain untriaged.
- **`.qa-r123-filtersem.mjs`** (do list filters actually narrow the result set?) written 05:04, **left no output file — whether it ran is unknown.**
- **Deliberately excluded, not oversight:** 🚫 **`/storm-map` address search is a STANDING EXCLUSION** — Google Places `AutocompleteSuggestion` + `Geocoder`, **billable per keystroke**; verified rendered and wired, not driven. 🚫 **FEMA map property code off-limits.** Paid/destructive `/leads/:id` controls (Measure Roof, Run Trace, Share Status Page, Remove Lead, Generate Contract). `/work-orders` milestone toggling and `/canvassing` pin drop (writes). **No screenshots taken, by choice.**

### DB hygiene — the "net zero" claim, scoped honestly
- Tenant-scoped counts for `791bb51d` (waterloo), whole night: `leads 13 · tasks 0 · estimates 16 · invoices 14 · work_orders 10 · contracts 4 · expenses 2 · subcontractors 63` — **byte-identical** to the pre-run snapshot.
- The API empty-body write sweep's **32-table count snapshot is identical before and after.**
- Only genuine side effect: the pipeline drag moved a lead `new → contacted → new`; only `updated_at` moved, which is unavoidable.
- 🚫 **STANDING TESTER-ERROR TRAP:** an **unscoped** `select count(*) from estimates` returns **17**, not 16 — the 17th row belongs to a different tenant. **Always scope count queries by `tenant_id` in a multi-tenant DB before reporting drift.**

### Notes for the next run
1. 🔑 **Triage the 62 tenant-scope candidates.** The only open item tonight that could be a security-class defect rather than a cosmetic or copy one.
2. 🔑 **Run `.qa-r123-taskfixture.mjs`.** One command clears a gap that has survived two nights of being written and abandoned.
3. 🔑 **Give s4 the public token routes.** Customer-facing, never swept, and the harnesses that fetch real tokens already exist.
4. ⚠️ **Stage capping is STILL the dominant failure mode, for the second night running** — s1, s3 and s4 all capped, the same three as 09-03. Tonight's defect count reflects how much testing ran, not how sound the app is. **Raise the s1/s3/s4 turn budgets or narrow their charters.**
5. 🔑 **Batching is what lets a stage finish.** s2 completed by collapsing work into few, wide calls. Never one call per route.
6. **"Write your artifact BEFORE spending the rest of your budget" was written last night and went unheeded** — s1 and s4 again left nothing, and s5 again spent budget reconstructing s1 from raw JSON. The one stage that wrote first (s2) needed no reconstruction.
7. **Read each stage's JSON `stop_reason`, never its self-assessment**, and **date-check `C:/tmp/*.txt` before treating an artifact as tonight's.** Both paid again: `/tmp/api-test-results.txt` is **Run 117's file from 2026-09-02**, and reading it as tonight's would have fabricated an entire section.
8. **Re-run `.qa-r124-undeclared.mjs` whenever a stage caps mid-edit.** Tonight proves a green build is not evidence that the app runs.
9. **Resynchronise the run number** — s2 and s3 disagreed by one on the same night.
10. **Decide on ESLint.** It is the structural answer to the whole class of defect that produced tonight's worst finding.
11. **Suggested targets** — **s1:** triage the 62 tenant-scope hits, then the task fixture, then the 7 singleton write routes. **s2:** the contract-template Edit path (candidate 404). **s3:** audits 1–7 converged again; spend the run on a *new* surface instead. **s4:** the three public token routes.
